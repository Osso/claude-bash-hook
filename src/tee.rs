...
//!
//! Auto-allows tee for files under /tmp/claude/
...
/// Allows writing to files under /tmp/claude/
pub fn check_tee(cmd: &Command) -> Option<PermissionResult> {
    if cmd.name != "tee" {
        return None;
    }
    ...
    // No files specified - passthrough (tee with no args just copies stdin to stdout)
    ...
    for path in &file_args {
        if !is_safe_tmp_claude_path(path) {
            // Not a safe /tmp/claude path - passthrough to normal handling
            return None;
        }
    }

    // All paths are safe /tmp/claude paths
    Some(PermissionResult {
        permission: Permission::Allow,
        reason: "tee to /tmp/claude".to_string(),
        suggestion: None,
    })
}
...
fn is_safe_tmp_claude_path(path: &str) -> bool {
                ...
                if let Some(parent_str) = parent.to_str() {
                    if !parent_str.is_empty() {
                        if let Some(resolved_parent) = resolve_path(parent_str) {
                            // Check if parent is under /tmp/claude
                            return is_under_tmp_claude(&resolved_parent);
                        }
                    }
                }
            }
    ...
    is_under_tmp_claude(&resolved)
}

/// Check if a resolved path is under /tmp/claude/
fn is_under_tmp_claude(resolved: &str) -> bool {
    // Must start with /tmp/claude/
    if !resolved.starts_with("/tmp/claude/") {
        // Also allow exactly /tmp/claude (the directory itself for writes)
        if resolved == "/tmp/claude" {
            return false; // Don't allow writing to the directory itself
        }
        return false;
    }

    // Must have something after /tmp/claude/
    let after = &resolved[12..]; // len("/tmp/claude/") = 12
    if after.is_empty() {
        return false;
    }

    // Reject if it's just /tmp/claude/ with trailing slashes
...
mod tests {
    ...
    fn test_tee_tmp_claude_file() {
        ...
        let result = check_tee(&cmd).unwrap();
        assert_eq!(result.permission, Permission::Allow);
    }

    #[test]
    fn test_tee_tmp_claude_subdir() {
        let cmd = make_cmd(&["/tmp/claude/logs/output.log"]);
        let result = check_tee(&cmd).unwrap();
        assert_eq!(result.permission, Permission::Allow);
    }

    #[test]
    fn test_tee_tmp_not_allowed() {
        let cmd = make_cmd(&["/tmp/test.log"]);
        let result = check_tee(&cmd);
    ...
    fn test_tee_with_append_flag() {
        let cmd = make_cmd(&["-a", "/tmp/claude/test.log"]);
        let result = check_tee(&cmd).unwrap();
        assert_eq!(result.permission, Permission::Allow);
    }
    ...
    fn test_not_tee_command() {
        let cmd = Command {
            name: "cat".to_string(),
            args: vec!["/tmp/claude/test.log".to_string()],
            text: "cat /tmp/claude/test.log".to_string(),
        };
        let result = check_tee(&cmd);
